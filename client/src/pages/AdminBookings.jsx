...
function AdminBookings() {
  ...
  const fetchAppointments = async (name) => {
    try {
      ...
      if (name) {
  const res = await fetch(`${API}/appoinment/by-staff/${encodeURIComponent(name)}`)
        if (!res.ok) {
  ...
  const updateStatus = async (id, action) => {
    try {
  const response = await fetch(`${API}/appoinment/${id}/${action}`, {
        method: 'PATCH'
      })
      if (response.ok) {
  ...
  const deleteAppointment = async (id) => {
    try {
  const response = await fetch(`${API}/appoinment/${id}`, {
        method: 'DELETE'
      })
      if (response.ok) {
