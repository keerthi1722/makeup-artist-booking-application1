...
function UserAppoinment() {
  ...
  useEffect(() => {
    const fetchBookedDates = async () => {
      try {
        const response = await fetch("http://localhost:5000/appoinment/booked-dates");
  ...
  const PostData = async (e) => {
    e.preventDefault();
    const { name, email, selectedServices, selectedSubServices, staff, date, totalPrice } = user;

    // Validation
    if (!name || !email || !selectedServices || selectedServices.length === 0 || !staff || !date) {
      window.alert("Please fill in all required fields and select at least one service.");
      return;
    }

    if (selectedSubServices.length === 0) {
      window.alert("Please select at least one sub-service.");
      return;
    }
    ...
    try {
      ...
      if (res.status === 201) {
        window.alert("Appointment successfully created! Check your email for confirmation.");
        // Reset form
        setUser({
          name: "",
          email: "",
        ...
        window.alert(data.error || data.message || "Failed to create appointment. Please try again.");
      }
    } catch (error) {
